<template>
  ...
  const dronePosition = window.scrollY + window.innerHeight / 2;

  serviceItems.forEach((item) => {
    const itemRect = item.getBoundingClientRect();
    const itemPosition = itemRect.top + window.scrollY + itemRect.height / 2;
    const distance = Math.abs(dronePosition - itemPosition);

    item.classList.remove('highlight'); // Убираем подсветку
  ...
  if (closestItem) {
    ...
    this.top = itemRect.top + window.scrollY + itemRect.height / 2;
    this.left = itemRect.left + window.scrollX + itemRect.width / 2;
    closestItem.classList.add('highlight'); // Добавляем подсветку
  }
},
    },
  };
